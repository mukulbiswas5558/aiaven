...
async def create_user_service(user: CreateUser):
    ...
    query = """
    INSERT INTO users (name, username, password, role, phone, department, employee_type, 
    ...
    """
    result = await db.fetchrow(query, user.name, user.username, hashed_password, user.role, user.phone, user.department, user.employee_type,
